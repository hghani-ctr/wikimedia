...
class Wikichart():
    ...
    def format(self, title, author=author, data_source="N/A", ybuffer=True, format_x_yearly=True, format_x_monthly=False, radjust=0.85, ladjust=0.1, tadjust=0.9, badjust=0.1, titlepad=0, perc=False):
        ...
        plt.figtext(0.1, 0.025, f"Graph Notes: Created by " + author + " " + " on " + str(date.today()) + " using data from " + str(data_source), family=style_parameters['font'], fontsize=8, color=wmf_colors['black25'])

    def annotate(self, x, y, num_annotation=None, legend_label="", label_color='black', num_color='black', xpad=0, ypad=0, zorder=10, use_last_y=False, perc=False):
        """Adds annotations at the end of a plotted line for the latest data point."""
        ...
        if legend_label:
            dynamic_spacing = 40 + len(legend_label) * 5
        else:
            dynamic_spacing = 10
    ...
    def finalize_plot(self, save_file_name, display=True):
        """Finalizes the plot by saving to file and optionally displaying it."""
    ...
    def format_subplots(self, title, key, author=author, data_source="N/A", radjust=0.85, ladjust=0.1, tadjust=0.85, badjust=0.1, num_charts=4, tickfontsize=12, mo_in_title=True):
        ...
        plt.figtext(0.05, 0.01, "Graph Notes: Created by " + str(author) + " " + str(date.today) + " using data from " + str(data_source), fontsize=8, va="bottom", ha="left", color=wmf_colors['black25'], fontproperties={'family': style_parameters['font']})
